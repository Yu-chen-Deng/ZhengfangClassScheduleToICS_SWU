...
function ClassScheduleToICS() {
    const timeMap = [
        ...
        { start: '12:10', end: '12:55' }, // 第5节
        { start: '13:05', end: '13:50' }, // 第6节
        { start: '14:00', end: '14:45' }, // 第7节
        { start: '14:55', end: '15:40' }, // 第8节
        { start: '15:50', end: '16:35' }, // 第9节
        { start: '16:55', end: '17:40' }, // 第10节
        { start: '17:50', end: '18:35' }, // 第11节
        { start: '19:20', end: '20:05' }, // 第12节
        { start: '20:15', end: '21:00' }, // 第13节
        { start: '21:10', end: '21:55' }, // 第14节
    ];
    ...
    // --------------------------------------------------------------------------
    // 更精确的确定时间
    function getClassStartTime(classNum) {
        const time = timeMap[classNum];
        if (!time) return '080000'; // 默认处理异常情况
        return time.start.replace(/:/g, '') + '00'; // 格式化为HHMMSS
    }

    function getClassEndTime(classNum) {
        const time = timeMap[classNum];
        if (!time) return '084500';
        return time.end.replace(/:/g, '') + '00';
    }
    ...
    function generateCalendar(courses) {
        ...
        courses.forEach(course => {
            for (let i = 0; i < course.isSingleOrDouble.length; ++i) {
                let e = new ICSEvent("" + getDate(course.startWeek[i], course.week) + getClassStartTime(course.startTime),
                    "" + getDate(course.startWeek[i], course.week) + getClassEndTime(course.endTime),
                    "" + course.name + " " + course.location + " " + course.teacher + " " + course.info);
                e.setRRULE("WEEKLY", res.Calendar.WKST,
                    "" + (course.endWeek[i] - course.startWeek[i] + course.isSingleOrDouble[i]) / course.isSingleOrDouble[i],
                    "" + course.isSingleOrDouble[i],
